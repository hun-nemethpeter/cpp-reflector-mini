...
namespace xpr {
   struct Name : pp_base {
      ...
      inline void print_resolution(const ipr::Name& name)
      {
         if (decl)
         {
            int at = pp.disambiguation_map.get_disambiguation(name, *decl);
            pp << '@' << at << ipr::token(" ");
         }
      }
      ...
      //--- conversion-function-id:
      ///        operator  type-id
      //--- \note This production is very different from ISO Standard
...
//--- primary-expression:
///           name           -- notice that use of a Decl is a "name"
///           label
///           type
///           ( expression )
...
namespace xpr {
   struct Initializer : xpr::Assignment_expr {
      Initializer(Printer& p) : xpr::Assignment_expr(p) { }
